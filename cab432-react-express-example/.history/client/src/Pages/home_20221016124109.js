...
const Home = () => {
  ...
  comst[isSentiment, setIsSantiment] = useState(false)
  ...
  const sentimentAnalysis = () => {
    fetch(`http://${local}/sen/${tag}`).then(res => res.json())
      .then(res => {
        console.log(res)
        if(res.map(res=>res.sentiment)==="positive"){
          setPositiveTweets(res.text)
        }else  if(res.map(res=>res.sentiment)==="nagetive"){
          setPositiveTweets(res.text)
        }else{
          setEmotionlessTweet(res.text)
        }
          ...
          /> :
            <div>
              ...
              <List
                size="small"
                bordered
                dataSource={setEmotionlessTweet}
                renderItem={item => <List.Item>{item}</List.Item>}
              />
